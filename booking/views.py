...
def profile(request):
    ...
    active_bookings = Booking.objects.filter(user=request.user, is_canceled=False)
    cancelled_bookings = Booking.objects.filter(user=request.user, is_canceled=True)
